import { applyPropsToElement, createElement, generateId, Timer } from "../../util.js"
...
class Spacecraft {
  init() {
    ...
    this.selfDestructSystem.onActive(() => {
      ...
      this.broadcastStatusTimer.stop()
      this.radioTransmitterSystem.send(this.dcId, { status: "destroy", energyPercentage: this.energySystem.energyPercentage })
    })

    this.broadcastStatusTimer = new Timer(() => {
      this.radioTransmitterSystem.send(this.dcId, { status: this.dynamicalSystem.isRunning ? "start" : "stop", energyPercentage: this.energySystem.energyPercentage })
    })

    this.dynamicalSystem.onStart(() => {
      this.broadcastStatusTimer.start()
    })
  }
  ...
  constructor(orbit, dynamicalSystem, energySystem, dcId, radioReceiverFactory, radioTransmitterFactory) {
    this.id = generateId()
    this.selfDestructSystem = new SelfDestructSystem()
    this.radioReceiverSystem = radioReceiverFactory(this.id)
    this.radioTransmitterSystem = radioTransmitterFactory(this.id)
...
class SpacecraftUI extends Spacecraft {
  ...
  constructor(orbit, dynamicalSystem, energySystem, dcId, radioReceiverFactory, radioTransmitterFactory) {
    super(orbit, dynamicalSystem, energySystem, dcId, radioReceiverFactory, radioTransmitterFactory)
    this.initGUI()
  }
