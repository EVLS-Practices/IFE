...
import { RadioTransmitter } from "./communication/index.js"
import { SpacecraftManager } from "./spacecraft/index.js"
...
class Conductor {
  ...
  async launchNewSpacecraft() {
    ...
    const succ = await this.send(this.id, SpacecraftManager.id, Conductor.COMMANDS.LAUNCH_NEW_SPACECRAFT)
    if (succ) {
  ...
  startSpacecraft(id) {
    return this.send(this.id, id, Conductor.COMMANDS.START_SPACECRAFT)
  }

  stopSpacecraft(id) {
    return this.send(this.id, id, Conductor.COMMANDS.STOP_SPACECRAFT)
  }

  destroySpacecraft(id) {
    return this.send(this.id, id, Conductor.COMMANDS.DESTROY_SPACECRAFT)
  }

  send(senderId, receiverId, msg) {
    return this.transmitter.send(senderId, receiverId, msg)
  }

  constructor(mediator, maximumSpacecraftNum) {
    this.id = generateId()
    this.transmitter = new RadioTransmitter(this.id, mediator)
